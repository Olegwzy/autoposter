...
def reboot_vm():
    msg = "⚠️ GPT и VM API недоступны более 12 часов — выполняю перезапуск autoposter.service 🔁"
    log(msg)
    notify_telegram(msg)
    try:
        subprocess.run(["sudo", "systemctl", "restart", "autoposter.service"], check=True)
    except Exception as e:
        log(f"❌ Ошибка рестарта autoposter: {e}")
        notify_telegram(f"❌ Ошибка рестарта autoposter: {e}")
